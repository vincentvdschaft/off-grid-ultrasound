...
def admm_inverse_beamform(
    ...
    pixel_positions : np.ndarray
        The positions of the pixels in shape `(n_z, n_x, 2)`.
    ...
    ax_min: int, default=0
        ...
        discard the first few samples in the RF data. Note that the `raw_data`
    ...
    assert pixel_positions.ndim == 3, "pixel_positions must be a 3D array."
    assert (
        pixel_positions.shape[2] == 2
    ), "pixel_positions must have shape (n_z, n_x, 2)."
    assert raw_data.ndim == 2, "raw_data must be a 2D array of shape (n_ax, n_el)."
    n_ax, n_el = raw_data.shape
    ...
    # Derived parameters
    # ======================================================================================
    n_z = pixel_positions.shape[0]
    n_x = pixel_positions.shape[1]
    n_pixels = n_z * n_x
    pixel_positions = pixel_positions.reshape(n_pixels, 2)
    # raw_data = raw_data / jnp.clip(jnp.max(raw_data), 1, None)
    ...
    raw_data = jnp.array(raw_data, dtype=jnp.float32)
    ...
    # ==================================================================================
    construct_phi_cached = cache_outputs("temp/cache")(construct_phi)

    phi = construct_phi_cached(
        n_ax,
        n_el,
    ...
    for n in range(max_n_iterations):
        ...
        if method == "admm":
            ...
            sigma = estimate_sigma(denoise_input.reshape(n_z, n_x))

            # Perform the denoising
            v = denoise_nl_means(
                denoise_input.reshape(n_z, n_x),
            ...
            for _ in range(red_iterations):
                sigma = estimate_sigma(z.reshape(n_z, n_x))
                ...
                z = (
                    ...
                    * denoise_nl_means(
                    ...
                    + beta / (mu + beta) * u.reshape(n_z, n_x)
                    + 1 / (mu + beta) * lam.reshape(n_z, n_x)
                )
            v = z.flatten()
...
def admm_inverse_beamform_compounded(
    ...
    """
    # Check if the raw_data is a 3D array
    assert isinstance(raw_data, (np.ndarray, jnp.ndarray)) and raw_data.ndim == 3, (
        "raw_data must be a 3D array of shape (n_tx, n_ax, n_el). "
        f"Got shape {raw_data.shape}."
    )
    n_tx, n_ax, n_el = raw_data.shape
...
def compute_tof_difference(
    ...
    t_tx = (
        jnp.min(t0_delays + dist_to_elements / sound_speed + tx_apodization_iszero)
        + t_peak
    )

    # Compute the tof from the pixel to the receiving element
    t_rx = dist_to_elements[el] / sound_speed
...
def compute_phi_row(
    ...
    factor = jnp.where(dz > f_number * dx, 1.0, 0.0).astype(jnp.float32) * (
        0.5 + 0.5 * jnp.cos(jnp.pi * f_number * dx / dz)
    )
